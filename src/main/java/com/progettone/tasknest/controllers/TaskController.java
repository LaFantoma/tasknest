...
public class TaskController {
    ...
    public ResponseEntity<?> moveTask(@RequestBody TaskDtoRqsPosition dto) {
        ...
        TaskList newTl = tlRepo.findByPositionAndBoardId(dto.getTasklistPosition(), dto.getBoardId()).get();

        if (dto.getPosition() > newTl.getTasks().size() + 1 || dto.getPosition() < 1) {
            return new ResponseEntity<String>("Invalid body request!", HttpStatus.BAD_REQUEST);
        }
        ...
        // trovo lista di task di destinazione
        List<Task> newTasks = tRepo.findAll().stream().filter(i -> i.getTasklist().equals(newTl)).toList();

        // se la lista di destinazione è la stessa di quella di partenza
        if (tl.getPosition().equals(newTl.getPosition())) {
        ...
        // se la lista di destinazione è un'altra
        else {

            newTasks.stream().filter(i -> i.getPosition() >= dto.getPosition()).forEach(e -> {
                e.setPosition(e.getPosition() + 1);
                tRepo.save(e);
            });
            tasks.stream().filter(i -> i.getPosition() > t.getPosition()).forEach(e -> {
                e.setPosition(e.getPosition() - 1);
                tRepo.save(e);
            });
        }
